...
type BydConfig = DictConfigs['byd']
...
export const search: SearchFunction<BydResult> = (
  text,
  config,
  profile,
  payload
) => {
  const bydConfig = profile.dicts.all.byd
  ...
  return fetchDirtyDOM(
    ...
    .then(async doc => {
      const searchResult: DictSearchResult<BydResult> = {
        result: {
          originText: 'i love BYD',
          result: '我爱BYD'
        }
      }
      return searchResult
